function centroids=centroids_WHC_assign(centroids, check_plots, force_recalc)
% Assign available water holding capacity (WHC) values to given centroids
% MODULE:
%   flood
% NAME:
%	centroids_WHC_assign
% PURPOSE:
...
% EXAMPLE:
%   centroids = centroids_WHC_assign(centroids)
% INPUTS:
...
%   'water_holding_capacity', which contains the annual mean available 
...
if ~exist('centroids','var') || isempty(centroids)
    cprintf([206 50 50]/255,['Error: Missing input ''centroids'' to '...
        'function centroids_WHC_assign, can''t proceed.\n']);
    return;
end
...
if ~isfield(centroids,'water_holding_capacity') || force_recalc
    ...
    for centroid_i=1:n_centroids
        ...
        centroids.water_holding_capacity(centroid_i) = ...
